...
public:
  ...
  forward(torch::autograd::AutogradContext *ctx, torch::Tensor input, int d_k,
    ...
    // temp2: (torch::matmul(output.transpose(1, 2), d_output, WF2.transpose(-2,
    // -1))) LayerNorm
  ...
  backward(torch::autograd::AutogradContext *ctx,
    ...
    torch::Tensor input = saved[0],
                  d_input =
                      torch::zeros({B, L, D}, torch::dtype(torch::kDouble));
    // puts("PASSED #0");
    torch::Tensor WQ = saved[1],
                  d_WQ = torch::zeros({D, D}, torch::dtype(torch::kDouble));
    // puts("PASSED #0");
    torch::Tensor BQ = saved[2],
                  d_BQ = torch::zeros({D}, torch::dtype(torch::kDouble));
    torch::Tensor WK = saved[3],
                  d_WK = torch::zeros({D, D}, torch::dtype(torch::kDouble));
    torch::Tensor BK = saved[4],
                  d_BK = torch::zeros({D}, torch::dtype(torch::kDouble));
    torch::Tensor WV = saved[5],
                  d_WV = torch::zeros({D, D}, torch::dtype(torch::kDouble));
    torch::Tensor BV = saved[6],
                  d_BV = torch::zeros({D}, torch::dtype(torch::kDouble));
    // puts("PASSED #0");
    torch::Tensor WX = saved[7],
                  d_WX = torch::zeros({D, D}, torch::dtype(torch::kDouble));
    torch::Tensor BX = saved[8],
                  d_BX = torch::zeros({D}, torch::dtype(torch::kDouble));
    ...
    // d_output = (scores_sm * (torch::eye(d_k).expand({B, L, d_k, d_k}) -
    // scores_sm)) * d_output;
    //
