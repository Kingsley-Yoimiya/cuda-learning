...
__global__ void debubble_kernel_1(const int *a, int n, int *blockpre) {
  ...
  for (int i = 1; i < blockDim.x; i <<= 1) {
    if (threadIdx.x + i < blockDim.x)
      s[threadIdx.x + i] += s[threadIdx.x];
    __syncthreads();
  }
  if (threadIdx.x == 0)
    blockpre[blockid] = s[blockDim.x - 1];
}

__global__ void debubble_kernel_2(const int *a, int n, const int *blockpre,
                                  int *output) {
  __shared__ int s[BLOCK_SIZE];
  int st = (blockIdx.x * blockDim.x + threadIdx.x) * BATCH, ed = st + BATCH;
  int cnt = 0;
  ...
  s[threadIdx.x] = cnt;
  __syncthreads();
  for (int i = 1, totnum = blockDim.x >> 1; i < blockDim.x;
       i <<= 1, totnum >>= 1) {
...
__global__ void preSum(int *a, int tot, size_t *ans) {
  ...
  if (threadIdx.x == 0)
    ans[0] = s[blockDim.x - 1], s[blockDim.x - 1] = 0;
  for (int i = blockDim.x >> 1, totnum = 1; i > 0; i >>= 1, totnum <<= 1) {
    if (threadIdx.x < totnum) {
      int id1 = threadIdx.x * (i << 1) + i - 1, id2 = id1 + i;
      int tmp = s[id1];
...
size_t debubble(vector<int> &a) {
  ...
  int *blockpre;
  int *output;
  int n = a.size(), totcore = (n + BATCH - 1) / BATCH,
      blocknum = nextPow2((totcore + BLOCK_SIZE - 1) / BLOCK_SIZE);
  ...
  // illigal:
  // for(int i = 1; i < blocknum; i++) cout << i << endl, blockpre[i] +=
  // blockpre[i - 1];
  size_t *cu_ans, ans;
