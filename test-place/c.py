...
with open("lltm_cu.cpp", "r") as file:
    cpp_source = file.read()
with open("lltm.cu", "r") as file:
    cu_source = file.read()
...
class LLTM(torch.nn.Module):
    def __init__(self, input_features, state_size):
        ...
        self.weights = torch.nn.Parameter(
            torch.empty(3 * state_size, input_features + state_size)).cuda()
        self.bias = torch.nn.Parameter(torch.empty(3 * state_size)).cuda()
...
X = torch.randn(batch_size, input_features, requires_grad=True).cuda()
h = torch.randn(batch_size, state_size, requires_grad=True).cuda()
C = torch.randn(batch_size, state_size, requires_grad=True).cuda()
...
# Forward: 18.034 s | Backward 46.218 s